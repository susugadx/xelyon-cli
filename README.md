...
- **Read-Before-Write ガード**: `read_file` せずに `str_replace` / `write_file` / `grep_replace` を実行しようとするとブロック（AI の盲目的な編集を防止）
...
コマンド失敗時は AI にエラー内容をフィードバックし修正を続行します。
