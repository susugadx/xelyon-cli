...
- 💾 **タイムスタンプ付きバックアップ**: 編集時に複数世代のバックアップを自動作成（v0.34.0）
- 🗑️ **自動クリーンアップ**: 古いバックアップを自動削除（デフォルト5世代保持）
- ✏️ **安全な編集**: str_replaceツールでdiff表示と確認プロンプト
- 📝 **.gitignore自動管理**: 初回バックアップ時に`*.bak*`を自動追加（確認あり、v0.34.0）
...
会話履歴をLLMでサマリー化してトークン数を削減します:

```bash
> /compress         # デフォルト: 最新10件を保持
🗜️  会話を圧縮中...

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
🗜️  Compression Results / 圧縮結果
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

圧縮前: 45,230 tokens (38 messages)
圧縮後: 12,450 tokens (10 messages + 1 summary)
削減率: 72.5%

✅ 圧縮完了
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

> /compress 15     # 最新15件を保持
```

**仕組み**:
1. 最新N件のメッセージを保持（デフォルト10件）
2. それ以前のメッセージをLLMに送信
3. 重要な決定事項、作成/編集ファイル、残タスクを箇条書きサマリー化
4. サマリーを履歴先頭に挿入、元メッセージを削除

**活用シーン**:
- 長時間の会話でコンテキストウィンドウが限界に近づいたとき
- API料金を節約したいとき
- 会話の要点だけ残して整理したいとき

**設定** (`~/.xelyon/config.yaml`):
```yaml
compression:
  auto_compress: false      # 自動圧縮を有効化（デフォルト: 無効）
...
### /changes と /undo all コマンド

セッション中のファイル変更履歴を管理できます:
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  ...
  [4] 📦 移動 internal/api/utils.go
      時刻: 14:28:45 | ツール: move_file | Undo: ✅
      説明: utils.goを新しいディレクトリに移動

  [5] ✂️  行削除 README.md
      時刻: 14:30:00 | ツール: delete_lines | Undo: ✅

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
使い方:
  /undo           - 最後の変更を取り消し
  /undo all       - すべての変更を取り消し
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
**使い方**:
- `/changes`: セッション中のすべての変更を一覧表示
  - 変更種別（作成/編集/削除/移動など）をアイコンで表示
  - タイムスタンプとツール名を表示
  - Undoステータス（✅: バックアップあり、❌: バックアップなし）を表示
- `/undo`: 最後の変更を1つ取り消し（確認プロンプトあり）
- `/undo all`: すべての変更を一括取り消し（確認プロンプトあり）

**特徴**:
- 新しい変更から古い変更へ逆順で復元（依存関係を考慮）
- バックアップファイル（.bak）の存在を自動確認
- 個別のファイル復元が失敗しても処理を継続
- 変更履歴は最大10件まで保持（設定可能）
...
### バックアップ機能（タイムスタンプ付き複数世代保持）

ファイル編集時、XELYON CLIは自動的にタイムスタンプ付きバックアップを作成します。
...
**1. 複数世代保持**
- 同じファイルの複数バックアップを保持（デフォルト5世代）
- 新しいバックアップ作成時、古いものは自動削除
- 世代数は設定ファイルで変更可能

**2. .gitignore自動管理**
- 初回バックアップ作成時に確認プロンプト表示
- ユーザーが承認すると `.gitignore` に以下を追加:
  ```
  ...
  *.bak.*
  ```
- 一度追加/拒否したら再度聞かない
- リポジトリルートの `.gitignore` を自動検出

#### 設定 (`~/.xelyon/config.yaml`)

```yaml
backup:
  max_generations: 5  # 保持する世代数（デフォルト5）
```
...
→ 最も古いバックアップ (20260110_143000) を自動削除
```

#### 既存の Undo 機能との関係

- `/undo` コマンドは **最新のバックアップ** を使用して復元
- タイムスタンプ付きバックアップは複数世代を保持するため、手動で古い世代に戻すことも可能
- 例: `mv config.yaml.bak.20260110_143500 config.yaml`

**注意**:
- バックアップは **編集時のみ** 作成（読み取り専用ツールでは作成しない）
- Git管理外に置きたい場合は `.gitignore` への追加を推奨
