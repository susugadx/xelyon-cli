...
func TestRootCommand_ClaudeDoctorHelpShowsMinimalFlags(t *testing.T) {
	...
	for _, want := range []string{"--model", "--catalog-model", "--smoke", "--tool-smoke", "--image-smoke", "--thinking-smoke", "--web-search-smoke", "--capabilities", "--require-capability", "--print-request", "--timeout", "--json", "Diagnose Claude provider configuration", "exact Messages", "/v1/messages"} {
		if !strings.Contains(out.String(), want) {
			t.Fatalf("output = %q, want Claude doctor help substring %q", out.String(), want)
		}
	}
	for _, unwanted := range []string{"--retention-smoke", "--print-config"} {
