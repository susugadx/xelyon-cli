...
func TestNewPlanReviewDisplay_RendersReviewActions(t *testing.T) {
	p := NewPlanReviewDisplay().
		...
		AddStep(1, "Update review display", []string{"apply_patch"}, []string{"internal/ui/plan_review.go"})

	result := p.Render()
	for _, want := range []string{
		"Implementation Plan Review",
		"関連ファイル",
		"internal/ui/plan_review.go",
		"確認",
...
func TestNewPlanApprovalPromptRequest_UsesPlanReviewActions(t *testing.T) {
	...
	if req.ConfirmSubmitPolicy != PromptConfirmSubmitExplicit {
	...
	assertPlanReviewPromptOption(t, req.Options[0], "Approve", string(PromptActionYes))
	assertPlanReviewPromptOption(t, req.Options[1], "Request changes", string(PromptActionComment))
	assertPlanReviewPromptOption(t, req.Options[2], "Cancel", string(PromptActionNo))
	if !strings.Contains(req.Placeholder, "change") {
		t.Fatalf("Placeholder = %q, want feedback guidance", req.Placeholder)
	}
}

func assertPlanReviewPromptOption(t *testing.T, option PromptOption, label string, value string) {
	t.Helper()
