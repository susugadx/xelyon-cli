...
func TestFileOpHelpers_PlainOutput(t *testing.T) {
	...
	got := buf.String()
	if !strings.Contains(got, "write  test.txt") {
		t.Fatalf("FileOpHeader output = %q, want header text", got)
	}
	if !strings.Contains(got, "────") {
		t.Fatalf("FileOpDivider output = %q, want divider", got)
	}
	if !strings.Contains(got, "-2 / +5 (net +3)") {
		t.Fatalf("FileOpStatsLine positive net output = %q, want +3 summary", got)
	}
	if !strings.Contains(got, "-5 / +2 (net -3)") {
		t.Fatalf("FileOpStatsLine negative net output = %q, want -3 summary", got)
	}
	if !strings.Contains(got, "-3 / +3 (net 0)") {
		t.Fatalf("FileOpStatsLine zero net output = %q, want zero summary", got)
	}
	if !strings.Contains(got, "M file.txt (+5, -2)") {
		t.Fatalf("FileOpPathLine output = %q, want path line", got)
	}
}
...
func TestFileOpHelpers_NilWriter(t *testing.T) {
	...
	if os.Getenv("SHOULD_NOT_EXIST") != "" {
		t.Fatal("unexpected environment mutation")
	}
}
