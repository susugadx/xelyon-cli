...
var turnSupportForbiddenImportRules = []architectureImportRule{
	{
		importRoot: "github.com/susugadx/xelyon-cli/internal/agent",
		rule:       "internal/turnsupport must not import internal/agent; keep turn orchestration in internal/agent",
	},
	{
		importRoot: "github.com/susugadx/xelyon-cli/internal/tui",
		rule:       "internal/turnsupport must not import internal/tui; keep terminal concerns outside turn support",
	},
	{
		importRoot: "github.com/susugadx/xelyon-cli/internal/api",
		rule:       "internal/turnsupport must not import internal/api; keep provider payload concerns outside turn support",
	},
	{
		importRoot: "github.com/charmbracelet/bubbletea",
		rule:       "internal/turnsupport must not import Bubble Tea directly",
	},
	{
		importRoot: "github.com/charmbracelet/lipgloss",
		rule:       "internal/turnsupport must not import Lip Gloss directly",
	},
}
...
func TestArchitectureBoundaries(t *testing.T) {
	...
	packageRoot := filepath.Join(repoRoot, "internal", "turnsupport")
	...
	if err := filepath.WalkDir(packageRoot, func(path string, d fs.DirEntry, walkErr error) error {
		...
		for _, imported := range file.Imports {
			...
			rule, ok := violatedArchitectureImportRule(importPath, turnSupportForbiddenImportRules)
			if !ok {
				continue
			}
		...
		t.Fatalf("walk internal/turnsupport imports: %v", err)
	}
}
