...
func (cs *Screen) renderConfigDetailPane(width, height int) []string {
	if width <= 0 {
		return nil
	}
	...
	cs.renderConfigFieldSummary(addLine, field)
	cs.renderConfigDetailEditor(addLine, field)
...
func (cs *Screen) renderConfigFieldSummary(addLine func(string), field *config.ConfigField) {
...
func (cs *Screen) renderConfigDetailEditor(addLine func(string), field *config.ConfigField) {
	switch cs.editMode {
	case editSelect:
		cs.renderConfigSelectDetail(addLine, field)
	case editInput:
		addLine(theme.Config.FgCyan + "  Edit:")
		addLine("  " + theme.Config.FgBright + cs.editInput.View())
	case editSlice:
		cs.renderConfigSliceDetail(addLine)
	case editStructMap:
		cs.renderConfigStructMapDetail(addLine, field)
	default:
