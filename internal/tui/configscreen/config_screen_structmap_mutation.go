...
func (cs *Screen) structMapKeys(path string) []string {
	var keys []string
...
func (cs *Screen) deleteStructMapKey(path, key string) {
	val, _ := config.GetFieldValue(cs.cfg, path)
	switch v := val.(type) {
	case map[string]config.ProviderModelConfig:
...
func (cs *Screen) addStructMapKey(path, key string) bool {
...
func (cs *Screen) AddStructMapKey(path, key string) bool {
	...
	return cs.addStructMapKey(path, key)
}

// ensureStructMapInitialized は TUI が直接 mutate する struct map を必要に応じて初期化する。
func (cs *Screen) ensureStructMapInitialized(path string) {
	if cs == nil {
		return
	}
