...
func (cs *Screen) handleBrowseEscape() configCommand {
...
func (cs *Screen) navUp(layout configLayout) {
	switch cs.activePane {
	case paneCategory:
...
func (cs *Screen) navDown(layout configLayout) {
	switch cs.activePane {
	case paneCategory:
...
func (cs *Screen) navRight(layout configLayout) {
	switch cs.activePane {
	case paneCategory:
		cs.activePane = paneField
	case paneField:
...
func (cs *Screen) selectPreviousCategory() {
	if cs.catIndex <= 0 {
		return
	}
	cs.catIndex--
...
func (cs *Screen) selectNextCategory() {
...
func (cs *Screen) selectPreviousField(layout configLayout) {
...
func (cs *Screen) selectNextField(layout configLayout) {
	fields := cs.filteredFields()
...
func (cs *Screen) resetFieldSelection() {
	cs.fieldIndex = 0
	cs.fieldScroll = 0
}

// ensureFieldVisible は fieldIndex が可視範囲内に入るよう fieldScroll を調整する。
func (cs *Screen) ensureFieldVisible(visibleRows int) {
	if visibleRows <= 0 {
		return
	}
