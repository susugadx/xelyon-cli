...
func (cs *Screen) handleStructMapListEdit(msg tea.KeyMsg) (configCommand, tea.Cmd) {
	s := msg.String()
	switch {
...
func (cs *Screen) deleteSelectedStructMapKey() {
...
func (cs *Screen) startStructMapAddKey() {
	cs.editStructAdding = true
	cs.editStructInput.SetValue("")
	cs.editStructInput.Focus()
}

func (cs *Screen) openSelectedStructMapEntry() {
	if cs.editStructIndex < 0 || cs.editStructIndex >= len(cs.editStructKeys) {
		return
	}
	field := cs.selectedField()
