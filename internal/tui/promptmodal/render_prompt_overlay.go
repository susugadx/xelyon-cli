...
// ViewOverlay は prompt modal を base view に重ねた文字列を返す。
func (p *Screen) ViewOverlay(base string, width int, height int) string {
	if p == nil {
		return base
	}
	...
	panelLines := p.panelLines(panelWidth)
...
func (p *Screen) panelLines(width int) []string {
	palette := theme.Config
	lines := p.contentLines(width)
	for i, line := range lines {
...
func (p *Screen) contentLines(width int) []string {
	palette := theme.Config
	title := termtext.StripANSI(termtext.SanitizeSingleLineANSI(p.req.Title))
	if title == "" {
		title = promptDefaultTitle(p.req.Kind)
	}
	message := termtext.StripANSI(termtext.SanitizeSingleLineANSI(p.req.Message))
	lines := []string{
	...
	for i, opt := range options {
		...
		if p.req.Kind == uiprompt.PromptKindMultiChoice {
			if p.values[opt.value] {
				prefix = "  [x] "
			} else {
				prefix = "  [ ] "
			}
