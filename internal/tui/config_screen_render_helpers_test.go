...
func TestConfigStatusHint(t *testing.T) {
	tests := []struct {
		...
		{
			name: "filter mode",
			cs:   &configScreen{filterMode: true},
			want: "Enter:apply  Esc:cancel",
		},
		{
			name: "slice editor idle",
			cs:   &configScreen{editMode: editSlice},
			want: "a:add  d:delete  Enter:edit  Esc:done",
		},
		{
			name: "slice editor editing",
			cs:   &configScreen{editMode: editSlice, editSliceEditing: true},
			want: "Enter:confirm  Esc:cancel",
		},
		{
			name: "struct entry input",
			cs:   &configScreen{editMode: editStructMap, editEntryActive: true, editEntryFieldEdit: "input"},
			want: "Enter:confirm  Esc:cancel",
		},
		{
			name: "default browse",
