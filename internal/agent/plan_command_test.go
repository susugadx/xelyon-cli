...
func TestGeneratedHelpText_DescribesPlanningOnlyPlanMode(t *testing.T) {
	if !strings.Contains(GeneratedHelpText, "implementation happens on next normal turn") {
		t.Fatalf("expected planning-only help text, got %q", GeneratedHelpText)
	}
	if strings.Contains(GeneratedHelpText, "step-by-step") {
		t.Fatalf("expected execution wording to be removed from help text, got %q", GeneratedHelpText)
	}
}
