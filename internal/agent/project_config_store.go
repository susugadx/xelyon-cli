...
type projectConfigLoadFunc func(cwd string) *config.ProjectConfig
...
type ProjectConfigStore struct {
	...
	maxEntries int
	loadFn     projectConfigLoadFunc
}

var (
	defaultProjectConfigStore  = NewProjectConfigStore()
	defaultProjectConfigLoadFn = func(cwd string) *config.ProjectConfig {
...
func NewProjectConfigStoreWithLimit(maxEntries int) *ProjectConfigStore {
	return NewProjectConfigStoreWithDeps(maxEntries, nil)
}

func NewProjectConfigStoreWithDeps(maxEntries int, loadFn projectConfigLoadFunc) *ProjectConfigStore {
	if maxEntries <= 0 {
		maxEntries = defaultProjectConfigStoreMaxEntries
	}
	if loadFn == nil {
		loadFn = defaultProjectConfigLoadFn
	}
...
func (s *ProjectConfigStore) loadForCWD(cwd string) *config.ProjectConfig {
	if s == nil {
		return defaultProjectConfigLoadFn(cwd)
	}

	cwd = cleanProjectPath(cwd)
	path, ok, err := config.ResolveProjectConfigPathForDir(cwd)
	if err != nil {
		return s.loadFromDisk(cwd)
	}
	if !ok {
		s.mu.Lock()
		s.ensureCacheLocked()
...
func (s *ProjectConfigStore) loadFromDisk(cwd string) *config.ProjectConfig {
...
func (s *ProjectConfigStore) resetCacheLocked() {
	if s == nil {
		return
	}
