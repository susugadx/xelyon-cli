...
func (p *providerFacingHistoryMutationProbe) HasCachedResponseID() bool {
	return strings.TrimSpace(p.responseID) != ""
}

func (p *providerFacingHistoryMutationProbe) SetResponseID(id string) {
	p.responseID = strings.TrimSpace(id)
}

func (p *providerFacingHistoryMutationProbe) GetResponseID() string {
	return p.responseID
}

func (p *providerFacingHistoryMutationProbe) ChatWithTools(ctx context.Context, _ string, history []api.Message, _ string) (string, error) {
	p.captureContext(ctx)
	p.capture(history)
...
func (p *providerFacingHistoryMutationProbe) ChatWithImage(ctx context.Context, _ string, history []api.Message, userMessage string, image *api.ImageData, _ string) (string, error) {
	if image == nil {
...
func assertProviderRequestHistoryReductionApplied(t *testing.T, agent *Agent, provider *providerFacingHistoryMutationProbe, oldRead, wantCurrentPrompt string) {
	...
	if !provider.capturedResponseIDChainDisabled {
		t.Fatal("provider request context did not disable response ID chain after history replacement")
	}
