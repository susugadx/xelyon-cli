...
func TestHandleSpecialCommandForSurface_Matrix(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:        "classic bare attach warns",
			surface:     commandcatalog.CommandSurfaceClassic,
			input:       "/attach",
			wantHandled: true,
			wantText:    "/attach is available in TUI mode only",
		},
		{
			name:        "classic non-bare attach falls through",
			surface:     commandcatalog.CommandSurfaceClassic,
			input:       "/attach ./notes.txt",
			wantHandled: false,
		},
		{
			name:        "TUI attach command is local owned",
			surface:     commandcatalog.CommandSurfaceTUI,
			input:       "/attach ./notes.txt",
			wantHandled: false,
		},
		{
			name:        "classic bare detach warns",
			surface:     commandcatalog.CommandSurfaceClassic,
			input:       "/detach",
			wantHandled: true,
			wantText:    "/detach is available in TUI mode only",
		},
		{
			name:        "classic non-bare detach falls through",
			surface:     commandcatalog.CommandSurfaceClassic,
			input:       "/detach 1",
			wantHandled: false,
		},
		{
			name:        "TUI detach command is local owned",
			surface:     commandcatalog.CommandSurfaceTUI,
			input:       "/detach 1",
			wantHandled: false,
		},
		{
			name:        "classic bare detach-all warns",
			surface:     commandcatalog.CommandSurfaceClassic,
			input:       "/detach-all",
			wantHandled: true,
			wantText:    "/detach-all is available in TUI mode only",
		},
		{
			name:        "classic non-bare detach-all falls through",
			surface:     commandcatalog.CommandSurfaceClassic,
			input:       "/detach-all now",
			wantHandled: false,
		},
		{
			name:        "TUI detach-all command is local owned",
			surface:     commandcatalog.CommandSurfaceTUI,
			input:       "/detach-all",
			wantHandled: false,
		},
