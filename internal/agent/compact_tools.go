...
const (
	...
	// compactSearchCodeMinLines は search_code 結果を圧縮する最小行数。
	compactSearchCodeMinLines = 60
...
// compactSearchCode は search_code の結果を history 格納用に圧縮する。
// サマリー行 ("Found N match(es) in M file(s)") + pattern ごとのファイルヘッダー一覧を残し、
// 詳細なマッチ行を省略する。
...
func compactSearchCode(result string) string {
	...
	// multi-pattern mode detection: "━━ Pattern" の存在で判定
	isMulti := strings.Contains(result, "━━ Pattern")

	if isMulti {
		compactSearchCodeMulti(&b, lines)
	} else {
		compactSearchCodeSingle(&b, lines)
	}

	fmt.Fprintf(&b, "\n[Detailed match lines omitted for history compaction. Use search_code to re-search if needed]\n")

	return b.String()
}

// compactSearchCodeSingle は single-pattern の search_code 結果を圧縮する。
// manifest 形式（"— N matches" を含む行）もファイル一覧として保持する。
func compactSearchCodeSingle(b *strings.Builder, lines []string) {
	for _, line := range lines {
		...
		// smart/full mode file header
		if strings.HasPrefix(trimmed, "📄 ") {
			fmt.Fprintf(b, "  %s\n", trimmed)
			continue
		}
		// manifest mode line: "  filepath — N matches (...)"
		if isManifestLine(trimmed) {
			b.WriteString(line)
			b.WriteByte('\n')
			continue
		}
		// warnings (rg fallback, unsupported option, etc.)
		...
		// detailed match lines, context lines, "..." separators → skip
	}
}

// isManifestLine は manifest 形式のファイル行か判定する。
// formatManifestLine (search_code_format.go) が生成する形式に依存:
//
//	"  %-40s — %d matches (%s)"  (with block names)
//	"  %-40s — %d matches"       (without block names)
//
// formatter がこの形式を変更した場合は追随が必要。
func isManifestLine(line string) bool {
	return strings.Contains(line, " — ") && strings.Contains(line, " matches")
}

// compactSearchCodeMulti は multi-pattern の search_code 結果を圧縮する。
// pattern ヘッダーごとにファイルヘッダーと diagnostics をグルーピングして出力する。
// pattern-level の "No matches found" / "⚠️ Error:" / warnings も保持する。
func compactSearchCodeMulti(b *strings.Builder, lines []string) {
	// current pattern section のファイルヘッダーを蓄積
	...
	for _, line := range lines {
		...
		// pattern section header → flush previous pattern's files, then emit header
		...
		// file header (smart/full mode) → collect under current pattern
		if strings.HasPrefix(trimmed, "📄 ") {
			pendingFiles = append(pendingFiles, trimmed)
			continue
		}

		// manifest mode line → preserve as-is
		if isManifestLine(trimmed) {
			b.WriteString(line)
			b.WriteByte('\n')
			continue
		...
		// pattern-level diagnostics: preserve within their pattern group
		if trimmed == "No matches found" ||
			strings.HasPrefix(trimmed, "⚠️ Error:") ||
		...
		// truncation / tip → flush then emit
		if strings.HasPrefix(trimmed, "[Results truncated") || strings.HasPrefix(trimmed, "Tip:") {
			flushFiles()
			b.WriteString(line)
			b.WriteByte('\n')
			continue
		}
		// detailed match lines → skip
	}
	// flush remaining files from last pattern section
	flushFiles()
}
