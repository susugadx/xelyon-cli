...
var providerHistoryForbiddenImportRules = []architectureImportRule{
	{
		importRoot: "github.com/susugadx/xelyon-cli/internal/agent",
		rule:       "internal/providerhistory must not import internal/agent; keep Agent state and request side effects in internal/agent",
	},
	{
		importRoot: "github.com/susugadx/xelyon-cli/internal/tui",
		rule:       "internal/providerhistory must not import internal/tui; keep terminal concerns outside provider history projection",
	},
	{
		importRoot: "github.com/charmbracelet/bubbletea",
		rule:       "internal/providerhistory must not import Bubble Tea directly",
	},
	{
		importRoot: "github.com/charmbracelet/lipgloss",
		rule:       "internal/providerhistory must not import Lip Gloss directly",
	},
}
...
func TestArchitectureBoundaries(t *testing.T) {
	...
	packageRoot := filepath.Join(repoRoot, "internal", "providerhistory")
	...
	if err := filepath.WalkDir(packageRoot, func(path string, d fs.DirEntry, walkErr error) error {
		...
		for _, imported := range file.Imports {
			...
			rule, ok := violatedArchitectureImportRule(importPath, providerHistoryForbiddenImportRules)
			if !ok {
				continue
			}
		...
		t.Fatalf("walk internal/providerhistory imports: %v", err)
	}
}
