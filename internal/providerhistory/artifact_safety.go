...
const providerHistoryBareSecretKeyPattern = `password|passwd|secret|api[_-]?key|api\s+key|authorization|auth[_-]?token|access[_-]?token|refresh[_-]?token|id[_-]?token|session[_-]?token|client[_-]?secret|private[_-]?key|jwt|token`
