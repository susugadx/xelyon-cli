...
var ToolDescriptions = map[string]string{
	...
	// Search & Discovery
	"inspect_symbol": "Preferred for exact Go symbol names. Use instead of search_code+read_file when the target function/type/method name is already known. Returns definition body, callers, references, and related tests in one call. Summary mode (default) is compact; full mode expands limits. Returns line-numbered source for direct str_replace line-range edits. Go files only (Phase 1).",
