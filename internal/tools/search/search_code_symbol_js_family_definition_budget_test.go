...
func TestResolveJSSymbol_ExactBudgetMultipleDefinitionsKeepsStructuredMultiple(t *testing.T) {
	dir := setupMultiLangDir(t, jsFamilyDefinitionFiles("buildUser", jsFamilyDefinitionCandidateMatchLimit))
	...
	if len(result.AffectedFiles) != jsFamilyDefinitionCandidateMatchLimit {
		t.Fatalf("AffectedFiles len = %d, want %d", len(result.AffectedFiles), jsFamilyDefinitionCandidateMatchLimit)
	}
}
...
func TestExecuteSearchCodeArtifactWithConfig_JavaScriptStructuredImpactExactBudgetMultipleDefinitionsKeepsAmbiguous(t *testing.T) {
	...
	if len(artifact.Metadata.AffectedFiles) != jsFamilyDefinitionCandidateMatchLimit {
		t.Fatalf("AffectedFiles len = %d, want %d", len(artifact.Metadata.AffectedFiles), jsFamilyDefinitionCandidateMatchLimit)
	}
}

func TestResolveJSSymbol_IncompleteMultipleDefinitionCandidatesDefersToFallback(t *testing.T) {
	dir := setupMultiLangDir(t, jsFamilyDefinitionFiles("buildUser", jsFamilyDefinitionCandidateMatchLimit+1))
...
func jsFamilyDefinitionFiles(symbol string, count int) map[string]string {
	files := make(map[string]string, count)
	for i := 0; i < count; i++ {
		files[fmt.Sprintf("src/%03d.js", i)] = fmt.Sprintf("function %s() { return 'ok' }\n", symbol)
	}
	return files
}
