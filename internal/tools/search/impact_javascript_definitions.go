...
func structuredJavaScriptImpactDefinitionsFromCandidates(symbol string, opts SearchOptions, candidates jsFamilyDefinitionCandidates) []genericSymbolDef {
	...
	defs = append(defs, findStructuredJavaScriptCommonJSInlineDefinitionsFromMatches(symbol, opts, candidates.matches)...)
...
func findStructuredJavaScriptCommonJSInlineDefinitionsFromMatches(symbol string, opts SearchOptions, matches []genericSymbolMatch) []genericSymbolDef {
	defs := make([]genericSymbolDef, 0)
	parsedFiles := newJSFamilyASTParsedFileCache()
	defer parsedFiles.Close()
	...
	for _, match := range matches {
		...
		if name, kind, ok := parseJavaScriptCommonJSInlineDefinition(match.Content, symbol); ok {
			if !javaScriptCommonJSInlineDefinitionMatchInCode(match, opts, parsedFiles) {
				continue
			}
...
func javaScriptCommonJSInlineDefinitionMatchInCode(match genericSymbolMatch, opts SearchOptions, parsedFiles *jsFamilyASTParsedFileCache) bool {
	absPath := absoluteAffectedFilePath(match.File, opts, affectedFileSourceText)
	if absPath == "" {
		return true
	}
	parsed := parsedFiles.Parsed(absPath)
	if parsed == nil {
		return true
	}
	return !jsast.LineStartsInNonCodeWithParsed(parsed, match.Line)
}
