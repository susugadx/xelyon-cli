...
var (
	...
	structuredTypeScriptTSXImpactTarget = structuredTypeScriptImpactTarget{
		...
		implementation:                 true,
		structuredImpact:               true,
		nearbyTestCandidateDefinitions: structuredTypeScriptNearbyTestCandidates(".tsx"),
	}
	structuredTypeScriptImplementationImpactTarget = structuredTypeScriptImpactTarget{
		...
		structuredImpact:               true,
		implementation:                 true,
		nearbyTestCandidateDefinitions: structuredTypeScriptNearbyTestCandidates(".ts"),
	}
)
...
func structuredTypeScriptNearbyTestCandidates(sourceSuffix string) []structuredTypeScriptNearbyTestCandidate {
	if sourceSuffix == "" {
		return nil
	}
	locations := []structuredTypeScriptNearbyTestLocation{
		structuredTypeScriptNearbyTestSibling,
		structuredTypeScriptNearbyTestNestedTestsDir,
		structuredTypeScriptNearbyTestWorkspaceTestsDir,
	}
	nameSuffixes := []string{".test" + sourceSuffix, ".spec" + sourceSuffix}
	candidates := make([]structuredTypeScriptNearbyTestCandidate, 0, len(locations)*len(nameSuffixes))
	for _, location := range locations {
		for _, suffix := range nameSuffixes {
			candidates = append(candidates, structuredTypeScriptNearbyTestCandidate{
				location: location,
				suffix:   suffix,
			})
		}
