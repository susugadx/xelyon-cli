...
func resolveListDirRequest(cfg *config.Config, path string, depth int, allowedRoots []string, fileFilter string, workspaceRoot string, projectMap *repomap.ProjectMap, projectMapStateKey string, ignoreMode listDirIgnoreMode) (listDirRequest, string) {
	...
	absPath, errResult := resolveValidatedPathWithRoots(common.NewOutput(io.Discard, io.Discard), path, allowedRoots, "path is empty")
	if errResult != "" {
...
func resolveListDirFilterRoot(absPath, matcherRoot, workspaceRoot string) string {
	if root := normalizeWorkspaceRoot(workspaceRoot); root != "" && isPathWithinRoot(absPath, root) {
		return root
	}
	if root := normalizeWorkspaceRoot(matcherRoot); root != "" && isPathWithinRoot(absPath, root) {
		return root
	}
	return normalizeWorkspaceRoot(absPath)
}
