...
// ResolveValidatedPath は file tool 用 path validation を行う。
...
// ResolveValidatedPathWithRoots は allowed roots を考慮して file tool 用 path validation を行う。
...
// IsPathWithinRoot は target が root 自身または root 配下か返す。
