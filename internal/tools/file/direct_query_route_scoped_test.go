...
func TestPlanGatherContextDirectRoute_ScopedExactFilenameLookup(t *testing.T) {
	root := t.TempDir()
	...
	policy := GatherContextDirectRoutePolicy{
		ScopedPath: "pkg",
		FileFilter: "go",
	}

	scoped := PlanGatherContextDirectRoute(execCtx, "target.go", policy)
	if scoped.Kind != GatherContextDirectRouteOutcomeResolved {
		t.Fatalf("expected scoped exact filename lookup to resolve, got %q (%s)", scoped.Kind, scoped.Error)
	}
	if scoped.Route.Kind != GatherContextDirectRouteRead {
		t.Fatalf("scoped.Route.Kind = %q, want %q", scoped.Route.Kind, GatherContextDirectRouteRead)
	}
	if got := scoped.Route.RawEntries(); len(got) != 1 || got[0] != filepath.Join("pkg", "target.go") {
		t.Fatalf("unexpected scoped route entries: %+v", got)
	}

	ranged := PlanGatherContextDirectRoute(execCtx, "target.go:2-2", policy)
	if ranged.Kind != GatherContextDirectRouteOutcomeResolved {
	...
	explicitRelative := PlanGatherContextDirectRoute(execCtx, "./target.go", policy)
	if explicitRelative.Kind != GatherContextDirectRouteOutcomeResolved {
	...
	explicitRelativeRange := PlanGatherContextDirectRoute(execCtx, "./target.go:2-2", policy)
	if explicitRelativeRange.Kind != GatherContextDirectRouteOutcomeResolved {
	...
	batch := PlanGatherContextDirectRoute(execCtx, "impl.go,impl_test.go", policy)
	if batch.Kind != GatherContextDirectRouteOutcomeResolved {
	...
	ignored := PlanGatherContextDirectRoute(execCtx, "package.json", GatherContextDirectRoutePolicy{
		ScopedPath: ".",
		FileFilter: "json",
	})
	if ignored.Kind != GatherContextDirectRouteOutcomeResolved {
	...
	explicitIgnored := PlanGatherContextDirectRoute(execCtx, filepath.Join("node_modules", "dep", "package.json"), GatherContextDirectRoutePolicy{
		FileFilter: "json",
	})
	if explicitIgnored.Kind != GatherContextDirectRouteOutcomeResolved {
	...
	missing := PlanGatherContextDirectRoute(execCtx, "missing.go", policy)
	if missing.Kind != GatherContextDirectRouteOutcomeError {
		t.Fatalf("missing.Kind = %q, want %q", missing.Kind, GatherContextDirectRouteOutcomeError)
	}
	...
	missingRange := PlanGatherContextDirectRoute(execCtx, "missing.go:1-2", policy)
	if missingRange.Kind != GatherContextDirectRouteOutcomeError {
		t.Fatalf("missingRange.Kind = %q, want %q", missingRange.Kind, GatherContextDirectRouteOutcomeError)
	}
	...
	missingBatch := PlanGatherContextDirectRoute(execCtx, "impl.go,missing_test.go", policy)
	...
	ambiguous := PlanGatherContextDirectRoute(execCtx, "target.go", policy)
	if ambiguous.Kind != GatherContextDirectRouteOutcomeError {
		t.Fatalf("ambiguous.Kind = %q, want %q", ambiguous.Kind, GatherContextDirectRouteOutcomeError)
	}
...
func TestPlanGatherContextDirectRoute_ScopedRelativeTargets(t *testing.T) {
	root := t.TempDir()
	...
	filePolicy := GatherContextDirectRoutePolicy{
		ScopedPath: "pkg",
		FileFilter: "go",
	}
	file := PlanGatherContextDirectRoute(execCtx, "nested/impl.go", filePolicy)
	if file.Kind != GatherContextDirectRouteOutcomeResolved {
		t.Fatalf("expected scoped relative file query to resolve, got %q (%s)", file.Kind, file.Error)
	}
	if file.Route.Kind != GatherContextDirectRouteRead {
		t.Fatalf("file.Route.Kind = %q, want %q", file.Route.Kind, GatherContextDirectRouteRead)
	}
	...
	ranged := PlanGatherContextDirectRoute(execCtx, "nested/impl.go:2-2", filePolicy)
	if ranged.Kind != GatherContextDirectRouteOutcomeError {
	...
	dir := PlanGatherContextDirectRoute(execCtx, "nested/", GatherContextDirectRoutePolicy{ScopedPath: "pkg", FileFilter: "go"})
	if dir.Kind != GatherContextDirectRouteOutcomeError {
...
func TestPlanGatherContextDirectRoute_ScopedSoftAndExplicitDirectFileFilterContract(t *testing.T) {
	root := t.TempDir()
	pkgDir := filepath.Join(root, "pkg")
	...
	softReadme := PlanGatherContextDirectRoute(execCtx, "README.md", GatherContextDirectRoutePolicy{
		ScopedPath: "pkg",
		FileFilter: "go",
	})
	if softReadme.Kind != GatherContextDirectRouteOutcomeNone {
		t.Fatalf("expected soft bare basename to fall back when filter mismatches, got %q (%s)", softReadme.Kind, softReadme.Error)
	}

	readme := PlanGatherContextDirectRoute(execCtx, "./README.md", GatherContextDirectRoutePolicy{
		ScopedPath: "pkg",
		FileFilter: "go",
	})
	if readme.Kind != GatherContextDirectRouteOutcomeResolved {
	...
	readmeRange := PlanGatherContextDirectRoute(execCtx, "./README.md:1-1", GatherContextDirectRoutePolicy{
		ScopedPath: "pkg",
		FileFilter: "go",
	})
	if readmeRange.Kind != GatherContextDirectRouteOutcomeResolved {
	...
	impl := PlanGatherContextDirectRoute(execCtx, "impl.go", GatherContextDirectRoutePolicy{
		ScopedPath: "pkg",
		FileFilter: "go",
	})
	if impl.Kind != GatherContextDirectRouteOutcomeResolved {
	...
	batch := PlanGatherContextDirectRoute(execCtx, "./README.md,impl.go", GatherContextDirectRoutePolicy{
		ScopedPath: "pkg",
		FileFilter: "go",
	})
	if batch.Kind != GatherContextDirectRouteOutcomeResolved {
	...
	ambiguous := PlanGatherContextDirectRoute(execCtx, "README.md", GatherContextDirectRoutePolicy{
		ScopedPath: "pkg",
		FileFilter: "pkg/docs/*.md",
	})
	if ambiguous.Kind != GatherContextDirectRouteOutcomeResolved {
...
func TestPlanGatherContextDirectRoute_ExplicitVsSoftDirectContractMatrix(t *testing.T) {
	root := t.TempDir()
	...
	tests := []struct {
		...
		query      string
		policy     GatherContextDirectRoutePolicy
		wantKind   GatherContextDirectRouteOutcomeKind
		wantRoute  GatherContextDirectRouteKind
		wantReads  []string
		wantErrSub string
	}{
		{
			...
			execCtx:   rootExecCtx,
			query:     "./README.md",
			policy:    GatherContextDirectRoutePolicy{FileFilter: "md"},
			wantKind:  GatherContextDirectRouteOutcomeResolved,
			wantRoute: GatherContextDirectRouteRead,
		...
		},
		{
			...
			execCtx:  rootExecCtx,
			query:    "README.md",
			policy:   GatherContextDirectRoutePolicy{ScopedPath: "pkg", FileFilter: "go"},
			wantKind: GatherContextDirectRouteOutcomeNone,
		},
		{
			name:      "soft basename with matching scoped filter resolves scoped direct",
			execCtx:   rootExecCtx,
			query:     "README.md",
			policy:    GatherContextDirectRoutePolicy{ScopedPath: "pkg", FileFilter: "md"},
			wantKind:  GatherContextDirectRouteOutcomeResolved,
			wantRoute: GatherContextDirectRouteRead,
		...
		},
		{
			...
			query:      "../../outside.txt",
			policy:     GatherContextDirectRoutePolicy{FileFilter: "go"},
			wantKind:   GatherContextDirectRouteOutcomeError,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			outcome := PlanGatherContextDirectRoute(tt.execCtx, tt.query, tt.policy)
			if outcome.Kind != tt.wantKind {
				t.Fatalf("outcome.Kind = %q, want %q (%s)", outcome.Kind, tt.wantKind, outcome.Error)
			}
			if tt.wantKind == GatherContextDirectRouteOutcomeResolved {
			...
			if tt.wantKind == GatherContextDirectRouteOutcomeError && !strings.Contains(outcome.Error, tt.wantErrSub) {
