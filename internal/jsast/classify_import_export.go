...
func isRequireBindingReference(parsed *ParsedFile, node *gotreesitter.Node) bool {
	for current := node; current != nil; current = current.Parent() {
		...
		return (name != nil && nodeWithin(node, name)) ||
			nodeWithinRequireMemberImport(parsed, node, value)
	}
	return false
}

func nodeWithinRequireMemberImport(parsed *ParsedFile, node *gotreesitter.Node, value *gotreesitter.Node) bool {
	if nodeKind(parsed, value) != "member_expression" {
		return false
	}
	if _, ok := requireCallSource(parsed, childByField(parsed, value, "object")); !ok {
		return false
	}
	property := childByField(parsed, value, "property")
	return property != nil && nodeWithin(node, property)
}
