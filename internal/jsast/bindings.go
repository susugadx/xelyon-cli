...
func jsxLocalNameShadowsNamedImportAlias(parsed *ParsedFile, usageName *gotreesitter.Node, alias NamedImportAlias) bool {
	return jsxLocalNameShadowedByScopes(usageName, namedImportAliasShadowScopes(parsed, alias))
}
...
func namedImportAliasShadowScopes(parsed *ParsedFile, alias NamedImportAlias) []localBindingScopeRange {
	if parsed == nil || parsed.tree == nil || alias.Local == "" {
		return nil
	}
	...
	walkNamed(parsed.tree.RootNode(), func(node *gotreesitter.Node) {
		...
		if !ok || strings.TrimSpace(nodeText(parsed, nameNode)) != alias.Local {
			return
		}
		if nameNode.StartByte() == alias.localStartByte && nameNode.EndByte() == alias.localEndByte {
			return
		}
