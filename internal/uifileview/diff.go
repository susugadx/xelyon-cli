...
func newDiffPrinter(out io.Writer) diffPrinter {
	...
	return diffPrinter{out: out, pal: uistyle.NewFileOpPalette(out)}
}
...
func ShowColoredDiffWithRuntime(runtime outputRuntime, oldStr, newStr string, opts *DiffOptions) {
	ShowColoredDiffToWriter(runtimeOutput(runtime), oldStr, newStr, opts)
}
...
func ShowUnifiedDiffWithRuntime(runtime outputRuntime, diffOutput string) {
	ShowUnifiedDiffToWriter(runtimeOutput(runtime), diffOutput)
}
...
func fileOpDividerInternal(w io.Writer, pal uistyle.FileOpPalette, width int) {
	pal.Border(w, strings.Repeat("─", width))
	_, _ = fmt.Fprintln(w)
}

// fileOpStatsInternal はパッケージ内部用の変更統計描画。
func fileOpStatsInternal(w io.Writer, pal uistyle.FileOpPalette, removed, added int) {
	_, _ = fmt.Fprint(w, "  ")
...
func runtimeOutput(runtime outputRuntime) io.Writer {
	if runtime == nil {
		return stdio.Output()
	}
