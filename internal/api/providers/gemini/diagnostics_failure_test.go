...
func TestClassifyGeminiDiagnosticSmokeFailure(t *testing.T) {
	tests := []struct {
		...
		{
			name:           "thinking timeout",
			smoke:          geminiDiagnosticFailedSmoke("text", DiagnosticRouteStreamGenerateContentSSE, "thinking timeout: exceeded max retries (2): thinking timeout: no Gemini progress or actionable output received for 2m0s"),
			err:            errors.New("thinking timeout: exceeded max retries (2): thinking timeout: no Gemini progress or actionable output received for 2m0s"),
			wantKind:       geminiDiagnosticSmokeFailureKindTimeout,
			wantMessage:    "timed out",
			wantSuggestion: "streaming.thinking_timeout_seconds",
			notMessage:     "request failed",
		},
		{
			name:           "response start timeout",
			smoke:          geminiDiagnosticFailedSmoke("text", DiagnosticRouteStreamGenerateContentSSE, "response start timeout: exceeded max retries (1): response start timeout: no response from Gemini within the timeout period"),
			err:            errors.New("response start timeout: exceeded max retries (1): response start timeout: no response from Gemini within the timeout period"),
			wantKind:       geminiDiagnosticSmokeFailureKindTimeout,
			wantMessage:    "timed out",
			wantSuggestion: "--timeout",
			notMessage:     "request failed",
		},
		{
