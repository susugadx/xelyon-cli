...
func TestConvertFunctionCallToDisplayJSON(t *testing.T) {
	fc := &api.GeminiFunctionCall{
		Name:             "str_replace",
		Args:             map[string]any{"path": "/test.go", "old_str": "foo", "new_str": "bar"},
		ThoughtSignature: "abc123-long-encrypted-signature-data",
		ThoughtParts: []map[string]any{
			{"thought": true, "text": "thinking about the change"},
			{"thought_signature": "sig-only-part"},
		},
	}
	...
	// パース可能な JSON であること
	var parsed map[string]any
	if err := json.Unmarshal([]byte(displayJSON), &parsed); err != nil {
		t.Fatalf("Failed to parse display JSON: %v", err)
	}

	// tool 名が含まれること
	if parsed["tool"] != "str_replace" {
		t.Errorf("Expected tool=str_replace, got %v", parsed["tool"])
	}
	...
	if args["path"] != "/test.go" {
		t.Errorf("Expected args.path=/test.go, got %v", args["path"])
	}

	// thought_signature が含まれないこと
	if _, exists := parsed["thought_signature"]; exists {
		t.Error("display JSON should NOT contain thought_signature")
	}

	// thought_parts が含まれないこと
	if _, exists := parsed["thought_parts"]; exists {
		t.Error("display JSON should NOT contain thought_parts")
	}

	// 文字列にも含まれないことを二重確認
	if strings.Contains(displayJSON, "thought_signature") {
		t.Errorf("display JSON string should not contain 'thought_signature': %s", displayJSON)
	}
	if strings.Contains(displayJSON, "thought_parts") {
		t.Errorf("display JSON string should not contain 'thought_parts': %s", displayJSON)
	}
}

func TestConvertFunctionCallToDisplayJSON_NoMetadata(t *testing.T) {
	// メタデータなしの場合も正常動作すること
	fc := &api.GeminiFunctionCall{
		Name: "read_file",
	...
	// メタデータなしの場合は display と internal が同じになる
	if displayJSON != toolJSON {
		t.Errorf("Expected same output when no metadata, got display=%s, tool=%s", displayJSON, toolJSON)
	}
}
