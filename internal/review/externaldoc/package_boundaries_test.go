...
var reviewExternalDocForbiddenImportRules = []architectureImportRule{
	{
		importRoot: "github.com/susugadx/xelyon-cli/internal/agent",
		rule:       "internal/review/externaldoc must not import internal/agent; keep runner orchestration in internal/review",
	},
	{
		importRoot: "github.com/susugadx/xelyon-cli/internal/tui",
		rule:       "internal/review/externaldoc must not import internal/tui; keep terminal concerns outside external doc evidence",
	},
	{
		importRoot: "github.com/susugadx/xelyon-cli/internal/api",
		rule:       "internal/review/externaldoc must not import internal/api; keep provider payload concerns outside external doc evidence",
	},
	{
		importRoot: "github.com/charmbracelet/bubbletea",
		rule:       "internal/review/externaldoc must not import Bubble Tea directly",
	},
	{
		importRoot: "github.com/charmbracelet/lipgloss",
		rule:       "internal/review/externaldoc must not import Lip Gloss directly",
	},
}
...
func TestArchitectureBoundaries(t *testing.T) {
	...
	packageRoot := filepath.Join(repoRoot, "internal", "review", "externaldoc")
	...
	if err := filepath.WalkDir(packageRoot, func(path string, d fs.DirEntry, walkErr error) error {
		...
		for _, imported := range file.Imports {
			...
			rule, ok := violatedArchitectureImportRule(importPath, reviewExternalDocForbiddenImportRules)
			if !ok {
				continue
			}
		...
		t.Fatalf("walk internal/review/externaldoc imports: %v", err)
	}
}
