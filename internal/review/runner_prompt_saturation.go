...
Do not use shallow or empty related context/search evidence as proof of no impact. Raw web search results are discovery-only. Fetched external_doc snippets are citation-capable evidence, but are not automatically official documentation or confirmed external specs unless source domain, title, and content clearly support that claim. Disabled, failed, truncated, or inconclusive review web search evidence is not external spec coverage. If credibility, truncation, or evidence gaps prevent verification, reflect that as residual, unverified, or blocked within review_report.v2.`
}
