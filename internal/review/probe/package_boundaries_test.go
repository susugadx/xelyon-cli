...
var reviewProbeForbiddenImportRules = []architectureImportRule{
	{
		importRoot: "github.com/susugadx/xelyon-cli/internal/agent",
		rule:       "internal/review/probe must not import internal/agent; keep runner orchestration in internal/review",
	},
	{
		importRoot: "github.com/susugadx/xelyon-cli/internal/tui",
		rule:       "internal/review/probe must not import internal/tui; keep terminal concerns outside probe runtime",
	},
	{
		importRoot: "github.com/charmbracelet/bubbletea",
		rule:       "internal/review/probe must not import Bubble Tea directly",
	},
	{
		importRoot: "github.com/charmbracelet/lipgloss",
		rule:       "internal/review/probe must not import Lip Gloss directly",
	},
}
...
func TestArchitectureBoundaries(t *testing.T) {
	...
	packageRoot := filepath.Join(repoRoot, "internal", "review", "probe")
	...
	if err := filepath.WalkDir(packageRoot, func(path string, d fs.DirEntry, walkErr error) error {
		...
		for _, imported := range file.Imports {
			...
			rule, ok := violatedArchitectureImportRule(importPath, reviewProbeForbiddenImportRules)
			if !ok {
				continue
			}
		...
		t.Fatalf("walk internal/review/probe imports: %v", err)
	}
}
