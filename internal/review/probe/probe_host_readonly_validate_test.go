...
func TestHostReadOnlyValidateRequest_AppliesDefaultExecutionLimits(t *testing.T) {
	...
	dirs, err := prepareHostReadOnlyRuntimeDirs(t.TempDir())
	if err != nil {
		t.Fatalf("prepareHostReadOnlyRuntimeDirs() error = %v", err)
	}
	env := buildHostReadOnlyEnv(executor.baseEnv, repo, dirs)
