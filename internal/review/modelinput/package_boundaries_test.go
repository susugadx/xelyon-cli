...
var reviewModelInputForbiddenImportRules = []architectureImportRule{
	{
		importRoot: "github.com/susugadx/xelyon-cli/internal/review/evidence",
		rule:       "internal/review/modelinput must not import review evidence; receive rendered evidence markdown as input",
	},
	{
		importRoot: "github.com/susugadx/xelyon-cli/internal/review/artifact",
		rule:       "internal/review/modelinput must not import review artifacts; artifact save timing belongs to internal/review runner",
	},
	{
		importRoot: "github.com/susugadx/xelyon-cli/internal/agent",
		rule:       "internal/review/modelinput must not import internal/agent; keep runner/model orchestration outside deterministic input assembly",
	},
	{
		importRoot: "github.com/susugadx/xelyon-cli/internal/tui",
		rule:       "internal/review/modelinput must not import internal/tui; keep terminal concerns outside model input assembly",
	},
	{
		importRoot: "github.com/susugadx/xelyon-cli/internal/tuiagent",
		rule:       "internal/review/modelinput must not import internal/tuiagent; keep TUI adapters outside model input assembly",
	},
	{
		importRoot: "github.com/susugadx/xelyon-cli/internal/api",
		rule:       "internal/review/modelinput must not import internal/api; keep provider payload concerns outside review model input assembly",
	},
	{
		importRoot: "github.com/susugadx/xelyon-cli/internal/commandruntime",
		rule:       "internal/review/modelinput must not import command runtime packages",
	},
	{
		importRoot: "github.com/susugadx/xelyon-cli/internal/toolruntime",
		rule:       "internal/review/modelinput must not import tool runtime packages",
	},
	{
		importRoot: "github.com/susugadx/xelyon-cli/internal/providerdiag",
		rule:       "internal/review/modelinput must not import provider runtime packages",
	},
	{
	...
	},
	{
		...
		rule:       "internal/review/modelinput must not import provider/model catalog packages",
	},
	{
		importRoot: "github.com/charmbracelet/bubbletea",
		rule:       "internal/review/modelinput must not import Bubble Tea directly",
	},
	{
		importRoot: "github.com/charmbracelet/lipgloss",
		rule:       "internal/review/modelinput must not import Lip Gloss directly",
	},
}
...
func TestArchitectureBoundaries(t *testing.T) {
	...
	packageRoot := filepath.Join(repoRoot, "internal", "review", "modelinput")
	...
	if err := filepath.WalkDir(packageRoot, func(path string, d fs.DirEntry, walkErr error) error {
		...
		for _, imported := range file.Imports {
			...
			if importPath == parentReviewPackageImportPath {
				...
				t.Errorf("%s:%d imports %q; violates rule %q", filepath.ToSlash(relFile), position.Line, importPath, "internal/review/modelinput must not import parent internal/review")
				continue
			}
			rule, ok := violatedArchitectureImportRule(importPath, reviewModelInputForbiddenImportRules)
			if !ok {
				continue
			}
		...
		t.Fatalf("walk internal/review/modelinput imports: %v", err)
	}
}
