...
func validateReviewReportProbeOutcomesAgainstPlan(report ReviewReport, plan PlanScope, trustedProbeSummaries []ReviewProbeSummary) error {
...
func newReviewReportProbePlanLinkageIndex(plan PlanScope) reviewReportProbePlanLinkageIndex {
...
func validateReviewReportImpactSurfaceProbeOutcomeCoverage(coverage *ReviewReportScopeCoverage, plan PlanScope, trustedByID map[string]ReviewProbeSummary, linkage reviewReportProbePlanLinkageIndex) error {
	...
	for i, surface := range plan.ImpactSurfaces {
		...
		if surface.Status == PlanImpactSurfaceNeedsProbe || surface.Status == PlanImpactSurfaceUnverified {
...
func validateReviewReportCandidateRiskProbeOutcomeCoverage(coverage *ReviewReportScopeCoverage, plan PlanScope, trustedByID map[string]ReviewProbeSummary, linkage reviewReportProbePlanLinkageIndex) error {
	...
	for i, risk := range plan.CandidateRisks {
		...
		if risk.Status == PlanCandidateRiskNeedsProbe || risk.Status == PlanCandidateRiskUnverified {
