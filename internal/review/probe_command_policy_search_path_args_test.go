...
func TestCollectSearchCommandPathCandidates_Grep(t *testing.T) {
	tests := []struct {
		...
		{
			name: "recursive infers last positional as pattern",
			args: []string{"-R", "/etc", "needle"},
			want: []string{"/etc"},
		},
		{
			name: "directories recurse long option infers path before pattern",
			args: []string{"--directories=recurse", "/etc", "needle"},
			want: []string{"/etc"},
		},
		{
