...
func TestProbeRunner_RepoSandbox_ProcessSandboxBlocksOriginalRepoMutation(t *testing.T) {
	repo := newProbeTestRepo(t, withProbeTestRepoNoLargeFile())
	runner := NewProbeRunner(repo)
	...
	result, err := runner.Run(context.Background(), ReviewProbeRequest{
		...
		Files: []ReviewProbeFile{{
			...
			Content: "package main\n\nimport (\n\t\"fmt\"\n\t\"os\"\n)\n\n" +
				fmt.Sprintf("func main() { if err := os.WriteFile(%q, []byte(\"mutated\\n\"), 0o644); err == nil { panic(\"write unexpectedly succeeded\") }; fmt.Println(\"blocked\") }\n", filepath.Join(repo, "keep.txt")),
		}},
		Commands: []ReviewProbeCommand{
...
func TestProbeRunner_RepoSandbox_ProcessSandboxBlocksHostFileRead(t *testing.T) {
	repo := newProbeTestRepo(t, withProbeTestRepoNoLargeFile())
	runner := NewProbeRunner(repo)
	...
	result, err := runner.Run(context.Background(), ReviewProbeRequest{
		...
		Files: []ReviewProbeFile{{
			Path: "read_host.go",
			Content: "package main\n\nimport (\n\t\"fmt\"\n\t\"os\"\n)\n\n" +
				"func main() { if _, err := os.ReadFile(\"/etc/passwd\"); err == nil { panic(\"read /etc/passwd unexpectedly succeeded\") }; fmt.Println(\"blocked\") }\n",
		}},
		Commands: []ReviewProbeCommand{
			{Command: "go", Args: []string{"run", "read_host.go"}},
		},
	})
	if err != nil {
		t.Fatalf("Run() error = %v", err)
