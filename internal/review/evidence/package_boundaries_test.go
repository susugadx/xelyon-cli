...
var reviewEvidenceForbiddenImportRules = []architectureImportRule{
	{
		importRoot: "github.com/susugadx/xelyon-cli/internal/agent",
		rule:       "internal/review/evidence must not import internal/agent; keep runner orchestration in internal/review",
	},
	{
		importRoot: "github.com/susugadx/xelyon-cli/internal/tui",
		rule:       "internal/review/evidence must not import internal/tui; keep terminal concerns outside evidence collection",
	},
	{
		importRoot: "github.com/susugadx/xelyon-cli/internal/api",
		rule:       "internal/review/evidence must not import internal/api; keep provider payload concerns outside evidence collection",
	},
	{
		importRoot: "github.com/charmbracelet/bubbletea",
		rule:       "internal/review/evidence must not import Bubble Tea directly",
	},
	{
		importRoot: "github.com/charmbracelet/lipgloss",
		rule:       "internal/review/evidence must not import Lip Gloss directly",
	},
}
...
func TestArchitectureBoundaries(t *testing.T) {
	...
	packageRoot := filepath.Join(repoRoot, "internal", "review", "evidence")
	...
	if err := filepath.WalkDir(packageRoot, func(path string, d fs.DirEntry, walkErr error) error {
		...
		for _, imported := range file.Imports {
			...
			if importPath == parentReviewPackageImportPath {
				...
				t.Errorf("%s:%d imports %q; violates rule %q", filepath.ToSlash(relFile), position.Line, importPath, "internal/review/evidence must not import parent internal/review")
				continue
			}
			rule, ok := violatedArchitectureImportRule(importPath, reviewEvidenceForbiddenImportRules)
			if !ok {
				continue
			}
		...
		t.Fatalf("walk internal/review/evidence imports: %v", err)
	}
}
