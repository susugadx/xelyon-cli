...
var reviewArtifactForbiddenImportRules = []architectureImportRule{
	{
		importRoot: "github.com/susugadx/xelyon-cli/internal/agent",
		rule:       "internal/review/artifact must not import internal/agent; keep runner orchestration in internal/review",
	},
	{
		importRoot: "github.com/susugadx/xelyon-cli/internal/tui",
		rule:       "internal/review/artifact must not import internal/tui; keep terminal concerns outside artifact writing",
	},
	{
		importRoot: "github.com/charmbracelet/bubbletea",
		rule:       "internal/review/artifact must not import Bubble Tea directly",
	},
	{
		importRoot: "github.com/charmbracelet/lipgloss",
		rule:       "internal/review/artifact must not import Lip Gloss directly",
	},
}
...
func TestArchitectureBoundaries(t *testing.T) {
	...
	packageRoot := filepath.Join(repoRoot, "internal", "review", "artifact")
	...
	if err := filepath.WalkDir(packageRoot, func(path string, d fs.DirEntry, walkErr error) error {
		...
		for _, imported := range file.Imports {
			...
			rule, ok := violatedArchitectureImportRule(importPath, reviewArtifactForbiddenImportRules)
			if !ok {
				continue
			}
		...
		t.Fatalf("walk internal/review/artifact imports: %v", err)
	}
}
