...
func (e *repoSandboxExecutor) run(ctx context.Context, req ReviewProbeRequest) (result ReviewProbeResult) {
	// repo_sandbox は process sandbox 内で、copy 済み sandbox root だけを書き込み可能にする。
	// 元 repo は sandbox に bind しない。
	...
	for _, cmd := range runtime.request.commands {
		cmdResult := e.executeRepoSandboxCommand(ctx, cmd, runtime.request.timeout, runtime.request.maxOutputBytes, runtime.env, runtime.sandbox)
...
func (e *repoSandboxExecutor) executeRepoSandboxCommand(ctx context.Context, cmd repoSandboxCommand, timeout time.Duration, maxOutputBytes int64, env []string, sandbox probeProcessSandbox) ReviewProbeCommandResult {
