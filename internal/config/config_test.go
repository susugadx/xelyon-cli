...
func TestGetModelForProvider(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "gemini", provider: "gemini", want: "gemini-3.1-pro-preview"},
...
func TestDefaultConfig_Embedding(t *testing.T) {
	cfg := DefaultConfig()
	...
	if cfg.Embedding.Model != "qwen3-embedding:0.6b" {
		t.Errorf("Embedding.Model = %q, want %q", cfg.Embedding.Model, "qwen3-embedding:0.6b")
	}
...
func TestLoadConfig_EmbeddingSection(t *testing.T) {
	tmpDir := t.TempDir()
	t.Setenv("HOME", tmpDir)
	...
	embeddingYAML := `embedding:
  enabled: false
  model: "nomic-embed-text"
  base_url: "http://myhost:11434"
`
	configPath := filepath.Join(configDir, "config.yaml")
	if err := os.WriteFile(configPath, []byte(embeddingYAML), 0600); err != nil {
		t.Fatalf("Failed to write config file: %v", err)
	}
	...
	if cfg.Embedding.Enabled {
		t.Error("Embedding.Enabled should be false (from YAML)")
	}
	if cfg.Embedding.Model != "nomic-embed-text" {
		t.Errorf("Embedding.Model = %q, want %q", cfg.Embedding.Model, "nomic-embed-text")
	}
	if cfg.Embedding.BaseURL != "http://myhost:11434" {
		t.Errorf("Embedding.BaseURL = %q, want %q", cfg.Embedding.BaseURL, "http://myhost:11434")
	}
}

func TestLoadConfig_EmbeddingDefaults_WhenNotSet(t *testing.T) {
	tmpDir := t.TempDir()
	t.Setenv("HOME", tmpDir)
	...
	// embedding セクションなし（enabled: true のみ）
	partialYAML := "embedding:\n  enabled: true\n"
	configPath := filepath.Join(configDir, "config.yaml")
	if err := os.WriteFile(configPath, []byte(partialYAML), 0600); err != nil {
		t.Fatalf("Failed to write config file: %v", err)
	}
	...
	// Model と BaseURL はデフォルト値が適用されること
	if cfg.Embedding.Model != "qwen3-embedding:0.6b" {
		t.Errorf("Embedding.Model = %q, want %q (default)", cfg.Embedding.Model, "qwen3-embedding:0.6b")
	}
	if cfg.Embedding.BaseURL != "http://localhost:11434" {
		t.Errorf("Embedding.BaseURL = %q, want %q (default)", cfg.Embedding.BaseURL, "http://localhost:11434")
	}
}
