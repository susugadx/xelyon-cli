...
func TestExtractJSArrowFunctionMetadata(t *testing.T) {
	tests := []struct {
		...
		{
			name:     "const async parenthesized single-param arrow",
			sig:      "const buildUser = async (id) => id",
			wantName: "buildUser",
			wantKind: "function",
			wantOK:   true,
		},
		...
		{
			name:     "const single-param arrow",
			sig:      "const buildUser = id => id",
			wantName: "buildUser",
			wantKind: "function",
			wantOK:   true,
		},
		{
			name:     "export const single-param arrow",
			sig:      "export const buildUser = id => id",
			wantName: "buildUser",
			wantKind: "function",
			wantOK:   true,
		},
		{
			name:     "const async single-param arrow",
			sig:      "const buildUser = async id => id",
			wantName: "buildUser",
			wantKind: "function",
			wantOK:   true,
		...
		{
			name:     "export const async single-param arrow",
			sig:      "export const buildUser = async id => id",
			wantName: "buildUser",
			wantKind: "function",
			wantOK:   true,
		},
		{
			name:     "async-prefixed param is not async keyword",
			sig:      "const buildUser = asyncValue => asyncValue",
			wantName: "buildUser",
			wantKind: "function",
			wantOK:   true,
		},
		{
			name:     "async parameter is not async keyword",
			sig:      "const buildUser = async => async",
			wantName: "buildUser",
			wantKind: "function",
			wantOK:   true,
		},
		{
			name:   "comparison expression is not single-param arrow",
			sig:    "const buildUser = id >= 1",
			wantOK: false,
		},
		{
		...
		{
			name:   "factory call is not arrow",
			sig:    "const buildUser = createBuilder()",
			wantOK: false,
		},
		{
