...
func TestStore_SnapshotReturnsDefensiveCopy(t *testing.T) {
	...
	recorder.SetLastPassedTests([]TestResult{NewTestResultWithExitCode("go test ./internal/taskstate", 0, "passed", "ok")})
	...
	if got := fresh.LastPassedTests.Results()[0].Command(); got != "go test ./internal/taskstate" {
		t.Fatalf("LastPassedTests command after snapshot mutation = %q, want %q", got, "go test ./internal/taskstate")
	}
