...
var taskStateForbiddenImportRules = []architectureImportRule{
	{
		importRoot: "github.com/susugadx/xelyon-cli/internal/agent",
		rule:       "internal/taskstate must not import internal/agent; keep turn orchestration in internal/agent",
	},
	{
		importRoot: "github.com/susugadx/xelyon-cli/internal/tui",
		rule:       "internal/taskstate must not import internal/tui; keep terminal concerns outside task state",
	},
	{
		importRoot: "github.com/susugadx/xelyon-cli/internal/api",
		rule:       "internal/taskstate must not import internal/api; keep task state provider-neutral",
	},
	{
		importRoot: "github.com/charmbracelet/bubbletea",
		rule:       "internal/taskstate must not import Bubble Tea directly",
	},
	{
		importRoot: "github.com/charmbracelet/lipgloss",
		rule:       "internal/taskstate must not import Lip Gloss directly",
	},
}
...
func TestArchitectureBoundaries(t *testing.T) {
	...
	packageRoot := filepath.Join(repoRoot, "internal", "taskstate")
	...
	if err := filepath.WalkDir(packageRoot, func(path string, d fs.DirEntry, walkErr error) error {
		...
		for _, imported := range file.Imports {
			...
			rule, ok := violatedArchitectureImportRule(importPath, taskStateForbiddenImportRules)
			if !ok {
				continue
			}
		...
		t.Fatalf("walk internal/taskstate imports: %v", err)
	}
}
