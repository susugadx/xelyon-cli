...
func TestRecorder_RecordToolObservation_BashTestsAndTouchedPaths(t *testing.T) {
	...
	recorder.RecordToolObservation(ToolObservation{
		...
		Result:   "ok\tgithub.com/susugadx/xelyon-cli/internal/taskstate\t0.01s\ninternal/taskstate/taskstate.go:12: ok",
	})
	recorder.RecordToolObservation(ToolObservation{
		ToolName: "bash",
		Args:     map[string]string{"command": "go test -v ./internal/taskstate"},
	...
	recorder.RecordToolObservation(ToolObservation{
		...
		Result:   "internal/taskstate/taskstate.go:11:type RuntimeTaskState struct {}",
	})

	snapshot := store.Snapshot()
	wantTouched := []string{"internal/taskstate/taskstate.go", "internal/agent/agent.go"}
	...
	if got := snapshot.LastPassedTests.Results(); len(got) != 2 ||
		got[0].Command() != "go test ./internal/taskstate" ||
		got[1].Command() != "go test -v ./internal/taskstate" ||
		got[1].ExitCode() != 0 {
...
func TestRecorder_RecordToolObservation_CancelledBashTestIsNotPassed(t *testing.T) {
	...
	recorder.RecordToolObservation(ToolObservation{
		ToolName: "bash",
		Args:     map[string]string{"command": "go test ./internal/taskstate"},
		Result:   "Command interrupted.\nPartial output:\nok\tgithub.com/susugadx/xelyon-cli/internal/taskstate\t0.01s",
	})
...
func TestRecorder_RecordToolObservation_WrappedBareBashTestCommands(t *testing.T) {
	store := NewStoreWithRoot(t.TempDir())
	recorder := store.Recorder()

	recorder.RecordToolObservation(ToolObservation{
		ToolName: "bash",
		Args:     map[string]string{"command": "cd internal/taskstate && go test"},
		Result:   "ok\tgithub.com/susugadx/xelyon-cli/internal/taskstate\t0.01s",
	})
	recorder.RecordToolObservation(ToolObservation{
		ToolName: "bash",
		Args:     map[string]string{"command": "time make ci-check"},
		Result:   "Error: exit status 2\nOutput: failed",
		Error:    true,
	})
	recorder.RecordToolObservation(ToolObservation{
		ToolName: "bash",
		Args:     map[string]string{"command": "printf 'go test'"},
		Result:   "go test",
	})

	snapshot := store.Snapshot()
	passed := snapshot.LastPassedTests.Results()
	if len(passed) != 1 || passed[0].Command() != "cd internal/taskstate && go test" {
		t.Fatalf("LastPassedTests = %#v, want wrapped bare go test", passed)
	}
	failed := snapshot.LastFailedTests.Results()
	if len(failed) != 1 || failed[0].Command() != "time make ci-check" || failed[0].ExitCode() != 2 {
		t.Fatalf("LastFailedTests = %#v, want wrapped bare make ci-check failure", failed)
	}
}
