...
func TestRecorder_RecordToolObservation_StructuredRecommendedReadsSkipRenderedFallback(t *testing.T) {
	root := t.TempDir()
	invocationCWD := filepath.Join(root, "pkg")
	for _, path := range []string{
		filepath.Join(root, "target.ts"),
		filepath.Join(invocationCWD, "target.ts"),
	} {
		if err := os.MkdirAll(filepath.Dir(path), 0o755); err != nil {
			t.Fatalf("mkdir test file dir %s: %v", path, err)
		}
		if err := os.WriteFile(path, []byte("export const target = true\n"), 0o644); err != nil {
			t.Fatalf("write test file %s: %v", path, err)
		}
	}
	store := NewStoreWithWorkspace(root, invocationCWD)
	...
	store.Recorder().RecordToolObservation(ToolObservation{
		...
		Result: strings.Join([]string{
			"Recommended reads:",
			"  - target.ts:7 | rendered ambiguous path",
		}, "\n"),
		Structured: &tools.RuntimeObservation{
			RecommendedReads: []tools.ObservationRecommendedRead{{
				Path:         "target.ts",
				ResolvedPath: filepath.Join(invocationCWD, "target.ts"),
				Reason:       "structured resolved path",
			}},
		},
	})

	reads := store.Snapshot().RecommendedReads.Items()
	if len(reads) != 1 ||
		reads[0].Path() != "pkg/target.ts" ||
		reads[0].Reason() != "structured resolved path" {
		t.Fatalf("RecommendedReads = %#v, want only structured pkg/target.ts read", reads)
	}
}
...
func TestRecorder_RecordToolObservation_SearchCodeExternalPathDoesNotRebaseRenderedFallback(t *testing.T) {
	root := t.TempDir()
	if err := os.WriteFile(filepath.Join(root, "foo.go"), []byte("package repo\n"), 0o644); err != nil {
		t.Fatal(err)
	}
	externalRoot := t.TempDir()
	externalFile := filepath.Join(externalRoot, "foo.go")
	...
	store.Recorder().RecordToolObservation(ToolObservation{
		...
		Args:       map[string]string{"path": externalRoot},
		Result: strings.Join([]string{
			"Found 1 match(es) in 1 file(s)",
			"",
			"📄 foo.go (1 match(es)) [L1]",
			"  [ref]     >   1 │ package external",
			"",
			"Recommended reads:",
			"  - foo.go:1 | external follow-up",
		}, "\n"),
		Structured: &tools.RuntimeObservation{
			TouchedFiles: []tools.ObservationPath{{
				Path:         "foo.go",
			...
			Evidence: []tools.ObservationEvidence{{
				...
				StartLine:    1,
				EndLine:      1,
				Excerpt:      "package external",
			}},
			RecommendedReads: []tools.ObservationRecommendedRead{{
				Path:         "foo.go",
				ResolvedPath: externalFile,
				Reason:       "external structured read",
			}},
		},
	})

	snapshot := store.Snapshot()
	if got := snapshot.TouchedFiles.Paths(); len(got) != 0 {
		t.Fatalf("TouchedFiles.Paths() = %v, want no repo records for external search output", got)
	}
	if got := snapshot.Evidence.Items(); len(got) != 0 {
		t.Fatalf("Evidence = %#v, want no repo evidence for external search output", got)
	}
	if got := snapshot.RecommendedReads.Items(); len(got) != 0 {
		t.Fatalf("RecommendedReads = %#v, want no repo reads for external search output", got)
	}
}

func TestRecorder_RecordToolObservation_GatherContextExternalPathDoesNotRebaseRenderedFallback(t *testing.T) {
	root := t.TempDir()
	if err := os.WriteFile(filepath.Join(root, "foo.go"), []byte("package repo\n"), 0o644); err != nil {
		t.Fatal(err)
	}
	externalRoot := t.TempDir()
	if err := os.WriteFile(filepath.Join(externalRoot, "foo.go"), []byte("package external\n"), 0o644); err != nil {
		t.Fatal(err)
	}
	...
	store.Recorder().RecordToolObservation(ToolObservation{
		...
		ToolCallID: "external-gather-search",
		Args:       map[string]string{"path": externalRoot},
		Result: strings.Join([]string{
			"Route: Auto search",
			"",
	...
	snapshot := store.Snapshot()
	if got := snapshot.TouchedFiles.Paths(); len(got) != 0 {
		t.Fatalf("TouchedFiles.Paths() = %v, want no repo records for external gather_context output", got)
	}
	if got := snapshot.Evidence.Items(); len(got) != 0 {
		t.Fatalf("Evidence = %#v, want no repo evidence for external gather_context output", got)
	}
	if got := snapshot.RecommendedReads.Items(); len(got) != 0 {
		t.Fatalf("RecommendedReads = %#v, want no repo reads for external gather_context output", got)
	}
}

func TestRecorder_RecordToolObservation_GatherContextExternalStructuredObservationSuppressesRenderedFallback(t *testing.T) {
	root := t.TempDir()
	if err := os.WriteFile(filepath.Join(root, "foo.go"), []byte("package repo\n"), 0o644); err != nil {
		t.Fatal(err)
	}
	...
	store.Recorder().RecordToolObservation(ToolObservation{
		ToolName:   "gather_context",
		ToolCallID: "inline-external-gather-search",
		Args:       map[string]string{"query": "Search for package under " + externalRoot},
		Result: strings.Join([]string{
			"Route: Auto search",
			"",
			"Found 1 match(es) in 1 file(s)",
	...
	snapshot := store.Snapshot()
	if got := snapshot.TouchedFiles.Paths(); len(got) != 0 {
		t.Fatalf("TouchedFiles.Paths() = %v, want no repo records when structured gather_context facts are external", got)
	}
	if got := snapshot.Evidence.Items(); len(got) != 0 {
		t.Fatalf("Evidence = %#v, want no repo evidence when structured gather_context facts are external", got)
	}
	if got := snapshot.RecommendedReads.Items(); len(got) != 0 {
		t.Fatalf("RecommendedReads = %#v, want no repo reads when structured gather_context facts are external", got)
	}
}

func TestRecorder_RecordToolObservation_SearchCodeAbsoluteRepoPathKeepsRenderedFallback(t *testing.T) {
	root := t.TempDir()
	scopedDir := filepath.Join(root, "pkg")
	if err := os.MkdirAll(scopedDir, 0o755); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(filepath.Join(scopedDir, "foo.go"), []byte("package pkg\n"), 0o644); err != nil {
		t.Fatal(err)
	}
	...
	store.Recorder().RecordToolObservation(ToolObservation{
		ToolName:   "search_code",
		ToolCallID: "repo-absolute-search",
		Args:       map[string]string{"path": scopedDir},
		Result: strings.Join([]string{
			"Found 1 match(es) in 1 file(s)",
			"",
			"📄 pkg/foo.go (1 match(es)) [L1]",
			"  [ref]     >   1 │ package pkg",
			"",
			"Recommended reads:",
			"  - pkg/foo.go:1 | repo follow-up",
		}, "\n"),
	})

	snapshot := store.Snapshot()
	if got := snapshot.TouchedFiles.Paths(); !reflect.DeepEqual(got, []string{"pkg/foo.go"}) {
		t.Fatalf("TouchedFiles.Paths() = %v, want [pkg/foo.go]", got)
	}
	evidence := snapshot.Evidence.Items()
	if len(evidence) != 1 || evidence[0].Path() != "pkg/foo.go" || evidence[0].StartLine() != 1 {
		t.Fatalf("Evidence = %#v, want pkg/foo.go:1", evidence)
	}
	reads := snapshot.RecommendedReads.Items()
	if len(reads) != 1 || reads[0].Path() != "pkg/foo.go" || reads[0].Reason() != "repo follow-up" {
		t.Fatalf("RecommendedReads = %#v, want repo follow-up", reads)
	}
}

func TestRecorder_RecordToolObservation_PrefersStructuredRecommendedReadsOverRenderedReads(t *testing.T) {
	fixture := newStructuredObservationRecorderFixture(t)
