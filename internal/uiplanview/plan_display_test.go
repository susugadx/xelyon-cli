...
func TestPlanDisplay_Render(t *testing.T) {
	p := NewPlanDisplay("Bracketed Paste Mode 対応").
		...
		AddStep(1, "multiline.go を修正", []string{"str_replace"}, []string{"internal/uiruntime/multiline.go"}).
...
func TestPlanDisplay_RenderStepReviewDetails(t *testing.T) {
	p := NewPlanDisplay("Plan Review").
		AddPlanStep(PlanStep{
			...
			Files:        []string{"internal/uiplanview/plan_display.go", "internal/uiplanview/plan_display.go", ""},
			Verification: []string{"go test ./internal/uiplanview", "go test ./internal/uiplanview"},
		})
	...
	for _, want := range []string{
		...
		"触るファイル: internal/uiplanview/plan_display.go",
		"検証: go test ./internal/uiplanview",
	...
	if strings.Contains(result, "go test ./internal/uiplanview, go test ./internal/uiplanview") {
