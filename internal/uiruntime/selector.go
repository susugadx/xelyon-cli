...
func selectorInputMatchesOption(input string, opt SelectOption) bool {
	return uiprompt.ConfirmPromptOptionMatchesInput(input, uiprompt.PromptOption(opt))
}
...
func selectorInputHint(options []SelectOption, requireExplicit bool) string {
	...
	for i, opt := range options {
		...
		if action, ok := uiprompt.ConfirmPromptActionFromValue(opt.Value); ok {
			switch action {
			case uiprompt.PromptActionYes:
				shortcuts = append(shortcuts, "y")
			case uiprompt.PromptActionNo:
