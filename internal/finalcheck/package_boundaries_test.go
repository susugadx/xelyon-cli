...
var finalCheckForbiddenImportRules = []architectureImportRule{
	{
		importRoot: "github.com/susugadx/xelyon-cli/internal/agent",
		rule:       "internal/finalcheck must not import internal/agent; keep command execution and turn orchestration in internal/agent",
	},
	{
		importRoot: "github.com/susugadx/xelyon-cli/internal/tui",
		rule:       "internal/finalcheck must not import internal/tui; keep terminal concerns outside final check policy",
	},
	{
		importRoot: "github.com/susugadx/xelyon-cli/internal/api",
		rule:       "internal/finalcheck must not import internal/api; keep provider payload concerns outside final check policy",
	},
	{
		importRoot: "github.com/charmbracelet/bubbletea",
		rule:       "internal/finalcheck must not import Bubble Tea directly",
	},
	{
		importRoot: "github.com/charmbracelet/lipgloss",
		rule:       "internal/finalcheck must not import Lip Gloss directly",
	},
}
...
func TestArchitectureBoundaries(t *testing.T) {
	...
	packageRoot := filepath.Join(repoRoot, "internal", "finalcheck")
	...
	if err := filepath.WalkDir(packageRoot, func(path string, d fs.DirEntry, walkErr error) error {
		...
		for _, imported := range file.Imports {
			...
			rule, ok := violatedArchitectureImportRule(importPath, finalCheckForbiddenImportRules)
			if !ok {
				continue
			}
		...
		t.Fatalf("walk internal/finalcheck imports: %v", err)
	}
}
