...
- `/config`: root tests currently inspect and mutate `configScreen` internals across many files. Move only with a matching test-boundary pass; do not add broad production getters/setters just to preserve old root tests.
- `/project`: direct root tests are fewer than `/config`, but save lifecycle tests mutate screen internals. Prefer moving pure screen behavior tests to `projectscreen` and keeping root tests on modal lifecycle/save command boundaries.
