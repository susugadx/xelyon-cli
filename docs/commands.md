...
3. **承認と実装**: ユーザーが計画を確認・承認すると、承認済み計画を通常モードへ引き継いで実装を開始

調査フェーズの履歴は通常実装へ持ち越さず、承認済み計画だけを通常モードの実装リクエストへ渡します。
...
| ツール名 | 説明 | 主な引数 |
|---------|------|---------|
| `search_code` | コード検索。`mode=auto` を既定に symbol-aware / literal / regex を language-aware に routing し、複数パターン・結果分類にも対応。対応言語では symbol-like query から定義・caller・参照・テストを自動解決。`intent=impact` は Go と TypeScript `.ts` で構造化 impact を優先し、それ以外は関連 multi-pattern 検索へフォールバック | `pattern`, `intent`, `mode`, `path`, `file_filter` 等 |
