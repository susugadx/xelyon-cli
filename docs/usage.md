...
XELYONはプロジェクト構造（RepoMap）をキャッシュして起動を高速化します。

**キャッシュの場所:**
`~/.xelyon/cache/repomap_<hash>.json`

**動作:**
- 初回起動時にキャッシュを生成
- 2回目以降は変更されたファイルのみ再解析
- git リポジトリでは `git status` と連携して効率的に差分検出

### トラブルシューティング

キャッシュに問題が発生した場合（起動エラー、古い情報が表示される等）、手動でクリアできます:

```bash
# 全プロジェクトのキャッシュをクリア
rm ~/.xelyon/cache/repomap_*.json

# キャッシュディレクトリの確認
ls -la ~/.xelyon/cache/
```

通常、キャッシュは自動管理されるため手動クリアは不要です。
