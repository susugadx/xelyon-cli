...
- **型**: string
- **デフォルト**: `""`（メインモデルを使用）
- **説明**: Supervisor用のモデル（調査クエリ生成、計画生成、エスカレーション判断）

##### `light_model`
- **型**: string
- **デフォルト**: `""`（メインモデルを使用）
- **説明**: Worker用の軽量モデル（通常のステップ実行）

##### `heavy_model`
- **型**: string
- **デフォルト**: `""`（エスカレーション無効）
- **説明**: エスカレーション用の高精度モデル（Worker失敗時に使用）
...
2. **エスカレーション**: `heavy_model` が設定されていれば高精度モデルで再実行
3. **ユーザー確認**: それでも失敗した場合、ユーザーに選択肢を提示
   - リトライ
   - コメント付きリトライ
   - スキップ
   - 中止
...
plan_mode:
  ...
  heavy_model: "claude-sonnet-4-5-20250514"
  max_retry: 5
```
